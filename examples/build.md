...
This document contains all the prompts and requirements needed to build the OneSignal Expo Sample App from scratch. Give these prompts to an AI assistant or follow them manually to recreate the app.
...
Run prebuild any time plugin/native config changes:
  npx expo prebuild
...
dependencies:
  ...
  react-native-screens: Expo-compatible version # Navigation peer dep
  react-native-safe-area-context: Expo-compatible version # Safe area handling
...
Install deps with Expo to avoid version drift:
...
Configure Metro to transform SVG files with Expo:
  // metro.config.js
...
No manual Podfile or android/app/build.gradle edits should be needed.
Let Expo prebuild + config plugins manage native wiring.
...
OneSignal.initialize(appIdFromExpoConfigOrStorage);
...
- All UI is React Native components running in Expo
...
- Native `ios` and `android` folders are generated by Expo prebuild when needed
...
These identifiers MUST be `com.onesignal.example` to work with the existing:
...
## Expo + React Native Best Practices Applied
...
- **Native changes via prebuild** instead of manual Podfile/Gradle edits
