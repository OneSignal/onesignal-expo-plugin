...
| `enableFirebaseInstallationIds` | optional       | Android only. If `true`, adds `firebase_messaging_installation_id_enabled=true` meta-data to the Android manifest so push registration uses the Firebase Installation ID (FID) path. Defaults to `false` (legacy FCM token path, manifest untouched). See [Firebase Installation ID (Android)](#firebase-installation-id-android).                                            |
...
### Firebase Installation ID (Android)

`enableFirebaseInstallationIds: true` is an opt-in for the Firebase Installation ID (FID) registration path. It only writes the manifest flag; the rest of the setup is owned by your app:

- Point `android.googleServicesFile` in your Expo config at the `google-services.json` for your own Firebase project. Every Android package variant (debug, release, flavors) must be registered in that project.
- Resolve `com.google.firebase:firebase-messaging` 25.1.0 or newer (for example via `expo-build-properties`). Firebase 25.x requires `minSdkVersion` 23.
- Use a `react-native-onesignal` release whose Android SDK supports FID registration.

Leaving the prop unset keeps the legacy FCM token path and requires none of the above. Because the flag is app-wide, another dependency can also enable it through manifest merging; if that happens you need the same setup even without this prop.
