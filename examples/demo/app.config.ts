...
const withWidgetPodfileTarget: ConfigPlugin = (cfg) =>
  withDangerousMod(cfg, [
    ...
    async (mod) => {
      ...
      // The widget Xcode target is hand-authored in the .xcodeproj (not
      // generated by any config plugin). Skip the Podfile block on fresh
      // prebuilds so `pod install` doesn't fail with "Unable to find a
      // target named OneSignalWidgetExtension". Once the target is added
      // back in Xcode, the next prebuild will append the block.
      const projectName = path.basename(
        (await fs.promises.readdir(platformRoot)).find((f) => f.endsWith('.xcodeproj')) ?? '',
        '.xcodeproj',
      );
      if (!projectName) return mod;
      const pbxproj = await fs.promises
        .readFile(path.join(platformRoot, `${projectName}.xcodeproj`, 'project.pbxproj'), 'utf8')
        .catch(() => '');
      if (!pbxproj.includes(WIDGET_TARGET_NAME)) {
        return mod;
      }
...
export default ({ config }: ConfigContext): ExpoConfig =>
  withWidgetPodfileTarget(
    withFastGradleProperties({
      ...
      experiments: {
        typedRoutes: true,
        // React Compiler with React 18 (SDK 52) requires `react-compiler-runtime`
        // and is currently published only as experimental builds. Re-enable
        // when upgrading back to SDK 54+/React 19, which ships the runtime
        // built into React itself.
        reactCompiler: false,
      },
      extra: {
