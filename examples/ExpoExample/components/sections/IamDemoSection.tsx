...
interface IamDemoSectionProps { loggingFunction: (message: string, optionalArg?: unknown) => void; }
...
export function IamDemoSection({ loggingFunction }: IamDemoSectionProps) {
  ...
  return (
    <View style={styles.container}>
      ...
      {IamTypes.map((type) => (
        <TouchableOpacity key={type.id} style={styles.button} onPress={() => handleIamDemo(type.id, type.title)}>
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: Colors.primary, borderRadius: 8, height: 44, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, marginTop: 8 },
  buttonText: { fontSize: 16, color: Colors.white, fontWeight: 'bold' },
  icon: { fontSize: 20, color: Colors.white, marginRight: 8 },
});
